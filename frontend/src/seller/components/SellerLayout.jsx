...
const NAV = [
  ...
  { id:'shipments', label:'Mes livraisons',   icon:'M1 3h15v13H1zM16 8h4l3 3v5h-7zM5.5 21a1.5 1.5 0 100-3 1.5 1.5 0 000 3zM18.5 21a1.5 1.5 0 100-3 1.5 1.5 0 000 3z' },
...
const SidebarContent = ({ seller, page, navigate, notifications, logout, onClose }) => {
      ...
      <div className="p-5 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {seller?.verified && <span className="text-xs text-blue-400">✓ Vérifié</span>}
          </div>
          {onClose && (
        ...
        {seller?.badge && (
          <div className="mt-2">
            <span className="text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-400 border border-blue-500/30">{seller.badge}</span>
          </div>
        )}
      </div>
        ...
        {NAV.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => { navigate(item.id); onClose?.(); }}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${active ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}>
              <SvgIcon path={item.icon} />
              <span className="flex-1 text-left">{item.label}</span>
              {badge > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${active ? 'bg-white/20' : 'bg-orange-500'} text-white`}>{badge}</span>
              )}
            </button>
          );
        })}
  ...
  return (
    <div className="flex h-screen bg-slate-950 overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-slate-900 border-r border-slate-800 flex-col flex-shrink-0">
        <SidebarContent seller={seller} page={page} navigate={navigate} notifications={notifications} logout={logout} />
      </aside>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-slate-900 flex flex-col shadow-2xl">
            <SidebarContent seller={seller} page={page} navigate={navigate} notifications={notifications} logout={logout} onClose={() => setSidebarOpen(false)} />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="h-14 bg-slate-900 border-b border-slate-800 flex items-center px-4 gap-3 flex-shrink-0">
          ...
          <h1 className="text-slate-200 font-semibold capitalize flex-1 text-sm md:text-base truncate">
